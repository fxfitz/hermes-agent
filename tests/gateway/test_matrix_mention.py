...
# ---------------------------------------------------------------------------
# MATRIX_DM_MENTION_THREADS — scoped sub-conversations within a single DM
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_dm_mention_thread_enabled_creates_thread(monkeypatch):
    """DM with @mention + MATRIX_DM_MENTION_THREADS=true → thread_id is set."""
    monkeypatch.setenv("MATRIX_REQUIRE_MENTION", "false")
    ...
    adapter = _make_adapter()
    room = _make_room(member_count=2)
    event = _make_event("@hermes:example.org help with this", event_id="$dm_thread1")

    await adapter._on_room_message(room, event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.source.thread_id == "$dm_thread1"
    assert "$dm_thread1" in adapter._bot_participated_threads


@pytest.mark.asyncio
async def test_dm_mention_thread_disabled_no_thread(monkeypatch):
    """DM with @mention + MATRIX_DM_MENTION_THREADS=false → thread_id stays None."""
    monkeypatch.setenv("MATRIX_REQUIRE_MENTION", "false")
    monkeypatch.setenv("MATRIX_DM_MENTION_THREADS", "false")

    adapter = _make_adapter()
    room = _make_room(member_count=2)
    event = _make_event("@hermes:example.org hello", event_id="$dm_no_thread")
...
@pytest.mark.asyncio
async def test_dm_no_mention_no_thread_created(monkeypatch):
    """DM without mention + MATRIX_DM_MENTION_THREADS=true → no thread (feature requires mention)."""
    monkeypatch.setenv("MATRIX_REQUIRE_MENTION", "false")
    monkeypatch.setenv("MATRIX_DM_MENTION_THREADS", "true")
    monkeypatch.delenv("MATRIX_AUTO_THREAD", raising=False)
    ...
    adapter = _make_adapter()
    room = _make_room(member_count=2)
    event = _make_event("hello there", event_id="$dm_no_mention")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    """DM thread created via MATRIX_DM_MENTION_THREADS is tracked like regular auto-threads."""
    monkeypatch.setenv("MATRIX_REQUIRE_MENTION", "false")
    monkeypatch.setenv("MATRIX_DM_MENTION_THREADS", "true")
    monkeypatch.delenv("MATRIX_AUTO_THREAD", raising=False)
    ...
    adapter = _make_adapter()
    room = _make_room(member_count=2)
    event = _make_event("@hermes:example.org start a thread", event_id="$dm_tracked")

    with patch.object(adapter, "_save_participated_threads"):
        await adapter._on_room_message(room, event)

    assert "$dm_tracked" in adapter._bot_participated_threads


@pytest.mark.asyncio
async def test_dm_mention_thread_default_disabled(monkeypatch):
    """Default: MATRIX_DM_MENTION_THREADS is false → no DM thread created."""
    monkeypatch.setenv("MATRIX_REQUIRE_MENTION", "false")
    ...
    adapter = _make_adapter()
    room = _make_room(member_count=2)
    event = _make_event("@hermes:example.org hello", event_id="$dm_default")
...
@pytest.mark.asyncio
async def test_dm_mention_thread_still_responds_without_mention(monkeypatch):
    """DMs always respond regardless of mention — thread_id just isn't set."""
    monkeypatch.setenv("MATRIX_REQUIRE_MENTION", "false")
    monkeypatch.setenv("MATRIX_DM_MENTION_THREADS", "true")
    monkeypatch.delenv("MATRIX_AUTO_THREAD", raising=False)
    ...
    adapter = _make_adapter()
    room = _make_room(member_count=2)
    event = _make_event("no mention here", event_id="$dm_always_responds")

    await adapter._on_room_message(room, event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "no mention here"
    assert msg.source.thread_id is None
