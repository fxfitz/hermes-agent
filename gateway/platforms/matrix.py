...
Environment variables:
    ...
    MATRIX_DM_MENTION_THREADS   In DMs, auto-create a thread when the bot is
                            mentioned via @bot:server. Each thread gets its own
                            session, enabling scoped sub-conversations within a
                            single DM. Default: false
"""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _on_room_message(self, room: Any, event: Any) -> None:
        ...
        has_mention = self._is_bot_mentioned(body, formatted_body)
        if not is_dm:
        ...
        # In DMs, start a thread when the bot is @mentioned — each thread = its own session.
        elif is_dm and not thread_id and has_mention:
            dm_mention_thread = os.getenv("MATRIX_DM_MENTION_THREADS", "false").lower() in ("true", "1", "yes")
            if dm_mention_thread:
                thread_id = event.event_id
                self._track_thread(thread_id)
