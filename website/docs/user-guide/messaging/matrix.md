...
| Context | Behavior |
|---------|----------|
| **DMs** | Hermes responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to enable scoped sub-conversations — when you `@mention` the bot in a DM, Hermes starts a thread and gives that thread its own session, so you can have multiple isolated conversations in the same DM. |
...
matrix:
  ...
  dm_mention_threads: false       # In DMs, start a thread on @mention (default: false)
```
